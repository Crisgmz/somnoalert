...
class _DrowsyPageState extends ConsumerState<DrowsyPage> {
  ...
  void initState() {
    ...
    _eventsSub = ref.read(drowsyControllerProvider.notifier).events.listen((
      event,
    ) {
      if (!mounted) return;
  ...
  void _onMetrics(
    ...
    next.whenData((metrics) {
      ...
      setState(() {
        ...
        if (video != null) {
          _cameraIndex =
              video.requestedIndex ?? video.activeIndex ?? _cameraIndex;
          _frameWidth =
              video.requestedWidth ?? video.activeWidth ?? _frameWidth;
          _frameHeight =
              video.requestedHeight ?? video.activeHeight ?? _frameHeight;
          _cameraFps = video.requestedFps ?? video.activeFps ?? _cameraFps;
          _cameraCodec =
              video.requestedCodec ?? video.activeCodec ?? _cameraCodec;
          _frameOrientation =
              video.requestedOrientation ??
              video.activeOrientation ??
              _frameOrientation;
        ...
        child: Column(
          ...
          children: [
            ...
            SwitchListTile.adaptive(
              ...
              subtitle: const Text(
                'Permite que el servidor active la alarma incluso sin audio local',
              ),
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('Video y Cámara', style: theme.textTheme.titleMedium),
              ],
            ),
            const SizedBox(height: 12),
  ...
  Future<void> _applyVideoConfig() async {
    ...
    final codec =
        (_cameraCodec == null || _cameraCodec!.toUpperCase() == 'AUTO')
        ? null
  ...
  factory _EventLogEntry.fromEvent(
    ...
    if (event is EyeRub) {
      ...
      return _EventLogEntry(
        ...
        subtitle:
            '${hand[0].toUpperCase()}${hand.substring(1)} · ${formatSeconds(event.duration)}',
